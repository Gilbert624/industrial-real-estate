...
with col1:
    ...
    if dd_projects:
        project_options = {f"{p.project_name} ({p.status})": p.id for p in dd_projects}
...
if st.session_state.selected_dd_project:
    ...
    with tab1:
        ...
        with st.form("dd_parameters_form"):
            ...
            with col1:
                name = st.text_input(f"{t('dd.parameters.project_name')}*", value=project.project_name)
            ...
            with col2:
                ...
                zoning = st.text_input(t('dd.parameters.zoning'), value=params.get('zoning', ''))
            ...
            with col1:
                land_area = st.number_input(
                    ...
                    min_value=0.0,
                    value=float(params.get('land_area_sqm', 0)),
                    step=100.0
                )
            ...
            with col2:
                building_area = st.number_input(
                    ...
                    min_value=0.0,
                    value=float(params.get('building_area_sqm', 0)),
                    step=100.0
                )
            ...
            with col2:
                acquisition_costs = st.number_input(
                    ...
                    min_value=0.0,
                    value=float(params.get('acquisition_costs', 0)),
                    step=10000.0,
                    format="%.0f",
            ...
            with col1:
                construction_cost = st.number_input(
                    ...
                    min_value=0.0,
                    value=float(params.get('construction_cost', 0)),
                    step=100000.0,
                    format="%.0f"
                )
                ...
                construction_duration = st.number_input(
                    ...
                    min_value=1,
                    value=int(params.get('construction_duration_months', 12)),
                    step=1
                )
            ...
            with col2:
                contingency = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=30.0,
                    value=float(params.get('contingency_percentage', 10)),
                    step=1.0,
            ...
            with col1:
                equity_pct = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=100.0,
                    value=float(params.get('equity_percentage', 30)),
                    step=5.0
                )
            ...
            with col2:
                debt_pct = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=100.0,
                    value=float(params.get('debt_percentage', 70)),
                    step=5.0,
                    disabled=True,
            ...
            with col3:
                interest_rate = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=20.0,
                    value=float(params.get('interest_rate', 6.0)),
                    step=0.25
                )
            ...
            loan_term = st.number_input(
                ...
                min_value=1,
                max_value=30,
                value=int(params.get('loan_term_years', 25)),
                step=1
            )
            ...
            with col1:
                monthly_rent = st.number_input(
                    ...
                    min_value=0.0,
                    value=float(params.get('estimated_monthly_rent', 0)),
                    step=5000.0,
                    format="%.0f"
                )
                ...
                rent_growth = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=20.0,
                    value=float(params.get('rent_growth_rate', 3.0)),
                    step=0.5
                )
            ...
            with col2:
                occupancy = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=100.0,
                    value=float(params.get('occupancy_rate', 95.0)),
                    step=5.0
                )
                ...
                opex_ratio = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=100.0,
                    value=float(params.get('operating_expense_ratio', 30.0)),
                    step=5.0,
            ...
            with col1:
                holding_period = st.number_input(
                    ...
                    min_value=1,
                    max_value=30,
                    value=int(params.get('holding_period_years', 10)),
                    step=1
                )
            ...
            with col2:
                exit_cap_rate = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=20.0,
                    value=float(params.get('exit_cap_rate', 6.5)),
                    step=0.25,
            ...
            if submitted or calculate:
                ...
                if not name:
                    ...
                    parameters = {
                        ...
                        'discount_rate': params.get('discount_rate', 8.0)  # 保留现有值
                    }
    ...
    with tab2:
        ...
        missing_params = []
        if not project.purchase_price:
            missing_params.append('purchase_price')
        if not params.get('construction_cost'):
            missing_params.append('construction_cost')
        if not params.get('estimated_monthly_rent'):
            missing_params.append('estimated_monthly_rent')
        ...
        if missing_params:
            ...
            # 准备参数（从 parameters JSON 中读取）
            model_params = {
                'purchase_price': project.purchase_price or 0,
                'acquisition_costs': params.get('acquisition_costs', 0),
                'construction_cost': params.get('construction_cost', 0),
                'construction_duration_months': params.get('construction_duration_months', 12),
                'contingency_percentage': params.get('contingency_percentage', 10.0),
                'equity_percentage': params.get('equity_percentage', 30.0),
                'debt_percentage': params.get('debt_percentage', 70.0),
                'interest_rate': params.get('interest_rate', 6.0),
                'loan_term_years': params.get('loan_term_years', 25),
                'estimated_monthly_rent': params.get('estimated_monthly_rent', 0),
                'rent_growth_rate': params.get('rent_growth_rate', 3.0),
                'occupancy_rate': params.get('occupancy_rate', 95.0),
                'operating_expense_ratio': params.get('operating_expense_ratio', 30.0),
                'holding_period_years': params.get('holding_period_years', 10),
                'exit_cap_rate': params.get('exit_cap_rate', 6.5)
            }
                    ...
                    if st.button("📄 Generate PDF Report", type="primary", use_container_width=True):
                        with st.spinner("Generating professional report..."):
                            try:
                                ...
                                # 提供下载
                                filename = f"DD_Report_{project.project_name.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.pdf"
    ...
    with st.form("new_dd_project_form"):
        ...
        if submitted:
            if not name or not location:
                ...
                try:
                    # 准备参数数据（使用默认值，后续可以在编辑页面修改）
                    parameters = {
                        'discount_rate': 8.0,  # 默认折现率 8%
                        'holding_period': 10,  # 默认持有期 10年
                        'rental_growth': 3.0,  # 默认租金增长率 3%
                        'exit_cap_rate': 6.0,  # 默认退出资本化率 6%
                        'description': ''  # 描述可以在编辑页面添加
                    }
                    ...
                    # 保存到数据库
                    project_id = db.add_dd_project(project_data)
                    
                    if project_id:
                        st.success(f"✅ Project '{name}' created successfully!")
                        st.info(f"Project ID: {project_id}")
                        st.balloons()
                        st.session_state.selected_dd_project = project_id
                        # 清空表单或重定向
                        time.sleep(1)
                        st.rerun()
                    else:
